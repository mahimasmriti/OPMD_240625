...
class  ImageShow(object):
    def __init__(self,
                ...
                test_auc_list,
                val_loss_list,val_acc_list,custom_path):        
    ...
    def train(self,opt="Loss",write=True,custom_path=None,img_title=None,suf=None,fold=None):
        if opt == 'Acc':
            img_portray(opt=opt,write=write,dates=self.trainacl,
                        location='upper left',
                        label='Train_Acc',col='red',
                        img_title=img_title,suf=suf, fold=fold)
        elif opt == 'Loss':
            img_portray(opt=opt,write=write,dates=self.trainll,
                        ...
                        img_title=img_title,suf=suf, fold=fold)
        if write:
            save_images(img_title=img_title,suf=suf,opt=opt, fold=fold)
        if SHOW_IMAGES:
        	plt.show()
        
    def test(self,opt='Acc',write=True,custom_path=None,img_title=None,suf=None, fold=None):
        if opt == 'Acc':
            ...
            img_portray(opt=opt,write=write,dates=self.testll,
                        ...
                        img_title=img_title,suf=suf, fold=fold)
        if write:
            save_images(split='test',img_title=img_title,suf=suf,opt=opt,fold=fold)
        if SHOW_IMAGES:
	        plt.show()
        
    def val(self,opt='Acc',write=True,custom_path=None,img_title=None,suf=None,fold=None):
        if opt == 'Acc':
            ...
            img_portray(opt=opt,write=write,dates=self.valll,
                        ...
                        img_title=img_title,suf=suf,fold=fold)
        if write:
            save_images(split='Val',img_title=img_title,suf=suf,opt=opt,fold=fold)
        if SHOW_IMAGES:
	        plt.show()
...
def save_images(split='train',custom_path='./tmp',img_title=None,suf=None,opt=None,fold=None):
    if os.path.isdir(f'{custom_path}/{img_title}/{suf}'):
        print (f'Image_path: {custom_path}/{img_title}/{suf}')
    else:
        os.makedirs(f'{custom_path}/{img_title}/{suf}')
        #!mkdir -p {custom_path}/{img_title}/{suf} #linux
    if split == 'train':
        plt.savefig(f'{custom_path}/{img_title}/{suf}/{split}_{opt}_{fold}.png',dpi=108)
        plt.clf()
    else:
        plt.savefig(f'{custom_path}/{img_title}/{suf}/{split}_{opt}_{fold}.png',dpi=128)
        plt.clf()
