...
ratio = 16 #8 #Feature_num/num_capsules or conv_outputs/num_capsules
...
class FixCapsNet(nn.Module):
    def __init__(self,conv_inputs,conv_outputs,
        ...
        #print('b4 conv',conv_inputs, conv_outputs)
        self.Convolution = make_features(cfgs[mode],f_c=conv_inputs,out_c=conv_outputs)
       
        #print('b4 cbam',conv_outputs)
        self.CBAM = Conv_CBAM(conv_outputs,conv_outputs)

        #print('b4 primarycaps',conv_outputs, primary_units)
        self.primary = Primary_Caps(in_channels=conv_outputs,
                                    caps_units=primary_units,
                                    )
        #print('b4 digitcaps', primary_units, primary_unit_size, num_classes, output_unit_size)       
        self.digits = Digits_Caps(in_units=primary_units,#16
    ...
    def forward(self, x):
        ...
        # print('DEBUG - conv out',x.device)
        x = self.CBAM(x)
        # print('DEBUG - cbam',x.device)
        x = self.primary(x)
        # print('DEBUG - primary',x.device)
        x = self.digits(x)
        # print('DEBUG - digits',x.device)
        return x
...
class Primary_Caps(nn.Module):
    def __init__(self, in_channels, caps_units):
        ...
        self.units = nn.ModuleList([create_conv_unit(i) for i in range(self.caps_units)])
    ...
    def forward(self, x):
        ...
        # print('DEBUG u',u.shape,u.device)
...
class Digits_Caps(nn.Module):
    ...
    def forward(self, x):
        ...
        b_ij = (torch.zeros(1, self.in_channels, self.num_units, 1)).to(device)
    ...
    def __init__(self, in_channels):
        ...
        self.Cpas = nn.Sequential(
                        nn.Conv2d(in_channels = in_channels,
                                  out_channels = Caps_out,
                                  kernel_size = 9,stride=2,groups=Caps_out),
                    )
